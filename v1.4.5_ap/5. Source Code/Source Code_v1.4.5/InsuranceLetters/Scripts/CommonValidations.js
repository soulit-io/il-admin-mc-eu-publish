...
CommonValidations.prototype.addCommonValidations = function () {
    ...
    jQuery.validator.addMethod("validateEmailAddress",
        function (value, element) {
            ...
            filter = /^([\w-\.]+)@((\[[0-9]{1,3}\.[0-9]{1,3}\.[0-9]{1,3}\.)|(([\w-]+\.)+))([a-zA-Z]{2,}|[0-9]{1,3})(\]?)$/;
    ...
    $.validator.addMethod("validatedatemaxyears",
        function (value, element) {            
            ...
            return (selectedDate.isAfter(todayDateB.subtract('years', 84)));
        });
    ...
    // CARDHOLDER AGE VALIDATION
    jQuery.validator.addMethod("validatecardholderage",        
        function (value, element) {
            var format = $(element).data('format');
            var years = moment().diff(moment(value, format), 'years');            

            return years >= 18;
        }
    );

    jQuery.validator.addMethod("validatecardholdermaxage",
        function (value, element) {
            var format = $(element).data('format');
            var years = moment().diff(moment(value, format), 'years');
            console.log("validando max: " + years);
            return years < 70;
        }
    );

    jQuery.validator.addMethod("validate_length_10",
        function (value, element) {
            return value.length == 10;
        }
    );

    jQuery.validator.addMethod("validate_start_5",
        function (value, element) {
    ...
    // CHILDS AGE VALIDATION
    jQuery.validator.addMethod("validatechildage",
        function (value, element) {
            var format = $(element).data('format');
            var years = moment().diff(moment(value, format), 'years');
            var months = moment().diff(moment(value, format), 'months');


            return years < 23 && months >= 6;
        }
    );
    ...
    jQuery.validator.addMethod("validateNotFutureDate", function (value, element) {
        var format = $(element).data('format');
        var ret = moment().diff(moment(value, format)) > 0;
        return ret;
    });

    // VALIDATIONS
    jQuery.validator.addMethod("validatedepartureafterpurchase",
        function (value, element) {
            var format = $(element).data('format');
            if (!value || value == '') return true; // NO VALUE, REQUIRED BUSINESS

            var purchaseStr = $('#PurchaseDateTrip').val();
            if (!purchaseStr || purchaseStr == '') return false; // NO PURCHASE, FAIL

            var departureDate = moment(value, format);
            var purchaseDate = moment(purchaseStr, format);

            return departureDate.isAfter(purchaseDate.add('days', -1));
        }
    );

    jQuery.validator.addMethod("validatereturnafterdeparture",
        function (value, element) {
            var format = $(element).data('format');
            if (!value || value == '') return true; // NO VALUE, REQUIRED BUSINESS
    ...
    jQuery.validator.addMethod("validatedepartureandreturnbetween180days",
        function (value, element) {
            var format = $(element).data('format');
            if (!value || value == '') return true; // NO VALUE, REQUIRED BUSINESS
            ...
            var returnDate = moment(value, format);
            var departureDate = moment(departureStr, format);
            var days = returnDate.diff(departureDate) / 86400000;

            return days <= 180;
        }
    );

    //Se debe actualizar el Core para que data devuelva la fecha de registración
    jQuery.validator.addMethod("validatedepartureandregistration",
        function (value, element) {
            var format = $(element).data('format');
            if (!value || value == '') return true; // NO VALUE, REQUIRED BUSINESS
            ...
            var registrationDate = moment(registration, format);
            var departureDate = moment(departureStr, format);
            var days = departureDate.diff(registrationDate) / 86400000;
            return days >= 0;
        }
    );


    jQuery.validator.addMethod("validatedestiny",
        function (value, element) {
            if (value == "CUB" || value == "IRN" || value == "PRK" ||
                value == "SYR" || value == "AFG" || value == "IRQ") {
                return false;
            }
            return true;
        }
    ...
    $.validator.addMethod("validatepurchaseaftergolive",
        function (value, element) {
            var format = $(element).data('format');
            if (!value || value == '') return true; // NO VALUE, REQUIRED BUSINESS
            var goLiveStr = $('#goLiveDate').val();
            if (!goLiveStr || goLiveStr == '') return false; // NO GO LIVE, FAIL
            var purchaseDate = moment(value, format);
            var goLiveDate = moment(goLiveStr, format);
            return purchaseDate.isAfter(goLiveDate.add('days', -1));
        });


    jQuery.validator.addMethod("validate_length_50",
        function (value, element) {
