    ...
    public class RenewHandler : StepHandler
    {
        ...
        public override JObject handleStep(JObject context)
        {
            ...
            if (! string.IsNullOrEmpty((string)context["system"]["renew_slug"]) )
            {
                ...
                context["system"]["renew_effective_date"] = (string)originalContext.SelectToken("$.input.fields[?(@.name=='ExpireDateTP')].values[0].value");
            }
            else
            {
                context["system"]["renew_effective_date"] = null;
            }
