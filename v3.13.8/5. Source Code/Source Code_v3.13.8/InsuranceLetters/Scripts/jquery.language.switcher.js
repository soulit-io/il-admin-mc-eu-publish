...
(function ($) {
	...
	$.fn.polyglotLanguageSwitcher = function (op) {
		...
		function init() {
			...
			var selectedItem;
			var options = $("#" + rootElementId + " > div > select > option");
			if (isStaticWebSite) {
			...
			options.each(function () {
				...
				var id = $(this).attr("id");
				if (Request.QueryString("cd")[1] != undefined) {
					if ($(this).attr("id") == Request.QueryString("cd")[1]) {
			...
			$("#" + rootElementId + " div:first-child").remove();
